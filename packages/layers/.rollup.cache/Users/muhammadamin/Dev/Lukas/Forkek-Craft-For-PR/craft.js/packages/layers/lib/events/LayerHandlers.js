import { DerivedCoreEventHandlers } from '@craftjs/core';
        ...
        return {
            layer: (el, layerId) => {
                ...
                const unbindDragOver = this.addCraftEventListener(el, 'dragover', (e) => {
                    ...
                    if (currentCanvasHovered &&
                        indicator &&
                        currentCanvasHovered.data.nodes) {
                        const heading = this.getLayer(currentCanvasHovered.id).headingDom.getBoundingClientRect();
                        if (e.clientY > heading.top + 10 &&
                            e.clientY < heading.bottom - 10) {
                            const currNode = currentCanvasHovered.data.nodes[currentCanvasHovered.data.nodes.length - 1];
                            if (!currNode) {
                                return;
                            }
                ...
                const unbindDragEnter = this.addCraftEventListener(el, 'dragenter', (e) => {
                    ...
                    if (!dragId)
                        return;
                    let target = layerId;
                    const indicatorInfo = editorStore.query.getDropPlaceholder(dragId, target, { x: e.clientX, y: e.clientY }, (node) => {
                        const layer = this.getLayer(node.id);
                        return layer && layer.dom;
                    });
                    if (indicatorInfo) {
                        const { placement: { parent }, } = indicatorInfo;
                        const parentHeadingInfo = this.getLayer(parent.id).headingDom.getBoundingClientRect();
            ...
            drag: (el, layerId) => {
                ...
                const unbindDragEnd = this.addCraftEventListener(el, 'dragend', (e) => {
                    ...
                    if (events.indicator && !events.indicator.error) {
                        ...
                        editorStore.actions.move(LayerHandlers.draggedElement, parentId, index + (where === 'after' ? 1 : 0));
                    }
...
//# sourceMappingURL=LayerHandlers.js.map